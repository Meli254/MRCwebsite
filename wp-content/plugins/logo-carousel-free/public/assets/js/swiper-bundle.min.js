! function(global, factory) {
    ...
    function _construct(Parent, args, Class) {
        return (_construct = _isNativeReflectConstruct() ? Reflect.construct : function _construct(Parent, args, Class) {
            var a = [null];
            a.push.apply(a, args);
            var Constructor, instance = new(Function.bind.apply(Parent, a));
    ...
    var Dom7 = function(_Array) {
        function Dom7(items) {
            var _this;
            return makeReactive(_assertThisInitialized(_this = _Array.call.apply(_Array, [this].concat(items)) || this)), _this
        }
        return _inheritsLoose(Dom7, _Array), Dom7
    ...
    function getTranslate(el, axis) {
        void 0 === axis && (axis = "x");
        var window = getWindow(),
            matrix, curTransform, transformMatrix, curStyle = getComputedStyle$1(el);
    ...
    function calcDevice(_temp) {
        var _ref, userAgent = (void 0 === _temp ? {} : _temp).userAgent,
            support = getSupport(),
            window = getWindow(),
        ...
        return !ipad && macos && support.touch && iPadScreens.indexOf(screenWidth + "x" + screenHeight) >= 0 && ((ipad = ua.match(/(Version)\/([\d.]+)/)) || (ipad = [0, 1, "13_0_0"]), macos = !1), android && !windows && (device.os = "android", device.android = !0), (ipad || iphone || ipod) && (device.os = "ios", device.ios = !0), device
    }

    function getDevice(overrides) {
        return void 0 === overrides && (overrides = {}), device || (device = calcDevice(overrides)), device
    }
    ...
    var supportsResizeObserver = function supportsResizeObserver() {
        ...
        Resize = {
            ...
            on: {
                init: function init(swiper) {
                    var window = getWindow();
                    swiper.params.resizeObserver && supportsResizeObserver() ? swiper.resize.createObserver() : (window.addEventListener("resize", swiper.resize.resizeHandler), window.addEventListener("orientationchange", swiper.resize.orientationChangeHandler))
                },
        ...
        Observer = {
            attach: function attach(target, options) {
                ...
                var window = getWindow(),
                    swiper = this,
                    ObserverFunc, observer = new(window.MutationObserver || window.WebkitMutationObserver)((function(mutations) {
        ...
        modular, eventsEmitter, update, translate, transition, slide, loop, grabCursor, manipulation;

    function updateSize() {
        var swiper = this,
            width, height, $el = this.$el;
        width = void 0 !== this.params.width && null !== this.params.width ? this.params.width : $el[0].clientWidth, height = void 0 !== this.params.height && null !== this.params.height ? this.params.height : $el[0].clientHeight, 0 === width && this.isHorizontal() || 0 === height && this.isVertical() || (width = width - parseInt($el.css("padding-left") || 0, 10) - parseInt($el.css("padding-right") || 0, 10), height = height - parseInt($el.css("padding-top") || 0, 10) - parseInt($el.css("padding-bottom") || 0, 10), Number.isNaN(width) && (width = 0), Number.isNaN(height) && (height = 0), extend(this, {
            width: width,
            height: height,
    ...
    function updateSlides() {
        var swiper = this,
            getDirectionLabel = function getDirectionLabel(property) {
            ...
            getDirectionPropertyValue = function getDirectionPropertyValue(node, label) {
        ...
        if (void 0 !== swiperSize) {
            ...
            for (var slidesPerColumn = params.slidesPerColumn, slidesPerRow = slidesNumberEvenToRows / slidesPerColumn, numFullColumns = Math.floor(slidesLength / params.slidesPerColumn), i = 0, newSlidesGrid, _$wrapperEl$css, _$wrapperEl$css2; i < slidesLength; i += 1) {
                slideSize = 0;
            ...
            if (0 === snapGrid.length && (snapGrid = [0]), 0 !== params.spaceBetween) {
                ...
                slides.filter(slidesForMargin).css(((_slides$filter$css = {})[key] = spaceBetween + "px", _slides$filter$css))
            }
    ...
    function updateSlidesOffset() {
        for (var swiper = this, slides = this.slides, i = 0; i < slides.length; i += 1) slides[i].swiperSlideOffset = this.isHorizontal() ? slides[i].offsetLeft : slides[i].offsetTop
    }
    ...
    function updateProgress(translate) {
        ...
        var params = this.params,
            ...
            wasEnd = isEnd;
        0 === translatesDiff ? (progress = 0, isBeginning = !0, isEnd = !0) : (isBeginning = (progress = (translate - this.minTranslate()) / translatesDiff) <= 0, isEnd = progress >= 1), extend(this, {
            progress: progress,
    ...
    function onTouchMove(event) {
        ...
        if (e.originalEvent && (e = e.originalEvent), data.isTouched) {
            if (!data.isTouchEvent || "touchmove" === e.type) {
                ...
                if (!this.allowTouchMove) return this.allowClick = !1, void(data.isTouched && (extend(touches, {
                    startX: pageX,
                    startY: pageY,
                    currentX: pageX,
                    currentY: pageY
                ...
                if (data.allowTouchCallbacks && this.emit("touchMove", e), !(e.targetTouches && e.targetTouches.length > 1)) {
                    ...
                    if (!(this.params.threshold && Math.sqrt(Math.pow(diffX, 2) + Math.pow(diffY, 2)) < this.params.threshold)) {
                        ...
                        if (data.isScrolling && this.emit("touchMoveOpposite", e), void 0 === data.startMoving && (touches.currentX === touches.startX && touches.currentY === touches.startY || (data.startMoving = !0)), data.isScrolling) data.isTouched = !1;
                            ...
                            if (params.touchReleaseOnEdges && (resistanceRatio = 0), diff > 0 && data.currentTranslate > this.minTranslate() ? (disableParentSwiper = !1, params.resistance && (data.currentTranslate = this.minTranslate() - 1 + Math.pow(-this.minTranslate() + data.startTranslate + diff, resistanceRatio))) : diff < 0 && data.currentTranslate < this.maxTranslate() && (disableParentSwiper = !1, params.resistance && (data.currentTranslate = this.maxTranslate() + 1 - Math.pow(this.maxTranslate() - data.startTranslate - diff, resistanceRatio))), disableParentSwiper && (e.preventedByNestedSwiper = !0), !this.allowSlideNext && "next" === this.swipeDirection && data.currentTranslate < data.startTranslate && (data.currentTranslate = data.startTranslate), !this.allowSlidePrev && "prev" === this.swipeDirection && data.currentTranslate > data.startTranslate && (data.currentTranslate = data.startTranslate), this.allowSlidePrev || this.allowSlideNext || (data.currentTranslate = data.startTranslate), params.threshold > 0) {
    ...
    function setBreakpoint() {
        ...
        if (breakpoints && (!breakpoints || 0 !== Object.keys(breakpoints).length)) {
            ...
            if (breakpoint && this.currentBreakpoint !== breakpoint) {
                ...
                directionChanged && initialized && this.changeDirection(), extend(this.params, breakpointParams), extend(this, {
    ...
    function addClasses() {
        var swiper = this,
            ...
            device = this.device,
            support = this.support,
            suffixes = prepareClasses(["initialized", params.direction, {
    ...
    var checkOverflow$1, defaults = {
            ...
            modular: {
                useParams: function useParams(instanceParams) {
                    ...
                    instance.modules && Object.keys(instance.modules).forEach((function(moduleName) {
                        ...
                        module.params && extend(instanceParams, module.params)
                    }))
                },
        ...
        Swiper = function() {
            function Swiper() {
                ...
                if (1 === args.length && args[0].constructor && "Object" === Object.prototype.toString.call(args[0]).slice(8, -1) ? params = args[0] : (el = args[0], params = args[1]), params || (params = {}), params = extend({}, params), el && !params.el && (params.el = el), params.el && $(params.el).length > 1) {
                    var swipers = [];
                ...
                return swiper.useParams(swiperParams), swiper.params = extend({}, swiperParams, extendedDefaults, params), swiper.originalParams = extend({}, swiper.params), swiper.passedParams = extend({}, params), swiper.params && swiper.params.on && Object.keys(swiper.params.on).forEach((function(eventName) {
                    swiper.on(eventName, swiper.params.on[eventName])
                })), swiper.params && swiper.params.onAny && swiper.onAny(swiper.params.onAny), swiper.$ = $, extend(swiper, {
                    el: el,
                    classNames: [],
                    slides: $(),
                    slidesGrid: [],
            ...
            return _proto.setProgress = function setProgress(progress, speed) {
                var swiper = this;
                progress = Math.min(Math.max(progress, 0), 1);
                var min = this.minTranslate(),
                    max, current = (this.maxTranslate() - min) * progress + min;
                ...
                var swiper = this,
                    ...
                    spv = 1;
                if (params.centeredSlides) {
                    for (var slideSize = slides[activeIndex].swiperSlideSize, breakLoop, i = activeIndex + 1; i < slides.length; i += 1) slides[i] && !breakLoop && (spv += 1, (slideSize += slides[i].swiperSlideSize) > swiperSize && (breakLoop = !0));
                ...
                var $el = $(el || this.params.el),
                    $wrapperEl;
                return !!(el = $el[0]) && (el.swiper = this, el && el.shadowRoot && el.shadowRoot.querySelector ? ($wrapperEl = $(el.shadowRoot.querySelector("." + this.params.wrapperClass))).children = function(options) {
                    return $el.children(options)
                } : $wrapperEl = $el.children("." + this.params.wrapperClass), extend(this, {
                    $el: $el,
                    el: el,
                    $wrapperEl: $wrapperEl,
                ...
                var swiper = this,
                    mounted;
                return this.initialized ? this : !1 === this.mount(el) ? this : (this.emit("beforeInit"), this.params.breakpoints && this.setBreakpoint(), this.addClasses(), this.params.loop && this.loopCreate(), this.updateSize(), this.updateSlides(), this.params.watchOverflow && this.checkOverflow(), this.params.grabCursor && this.setGrabCursor(), this.params.preloadImages && this.preloadImages(), this.params.loop ? this.slideTo(this.params.initialSlide + this.loopedSlides, 0, this.params.runCallbacksOnInit) : this.slideTo(this.params.initialSlide, 0, this.params.runCallbacksOnInit), this.attachEvents(), this.initialized = !0, this.emit("init"), this.emit("afterInit"), this)
                ...
                return void 0 === swiper.params || swiper.destroyed ? null : (swiper.emit("beforeDestroy"), swiper.initialized = !1, swiper.detachEvents(), params.loop && swiper.loopDestroy(), cleanStyles && (swiper.removeClasses(), $el.removeAttr("style"), $wrapperEl.removeAttr("style"), slides && slides.length && slides.removeClass([params.slideVisibleClass, params.slideActiveClass, params.slideNextClass, params.slidePrevClass].join(" ")).removeAttr("style").removeAttr("data-swiper-slide-index")), swiper.emit("destroy"), Object.keys(swiper.eventsListeners).forEach((function(eventName) {
            ...
            on: {
                beforeInit: function beforeInit(swiper) {
                    if (swiper.params.virtual.enabled) {
                        ...
                        extend(swiper.params, overwriteParams), extend(swiper.originalParams, overwriteParams), swiper.params.initialSlide || swiper.virtual.update()
                    }
                },
    ...
    var Mousewheel = {
            ...
            handle: function handle(event) {
                ...
                if (positions >= swiper.minTranslate() && (positions = swiper.minTranslate()), positions <= swiper.maxTranslate() && (positions = swiper.maxTranslate()), (disableParentSwiper = !!swiper.params.loop || !(positions === swiper.minTranslate() || positions === swiper.maxTranslate())) && swiper.params.nested && e.stopPropagation(), swiper.params.freeMode) {
                    var _newEvent = {
                            time: now(),
        ...
        Mousewheel$1, Navigation = {
            ...
            destroy: function destroy() {
                var swiper = this,
                    _swiper$navigation2 = this.navigation,
        ...
        Navigation$1, Pagination = {
            ...
            init: function init() {
                ...
                if (params.el) {
                    var $el = $(params.el);
                    0 !== $el.length && (swiper.params.uniqueNavElements && "string" == typeof params.el && $el.length > 1 && ($el = swiper.$el.find(params.el)), "bullets" === params.type && params.clickable && $el.addClass(params.clickableClass), $el.addClass(params.modifierClass + params.type), "bullets" === params.type && params.dynamicBullets && ($el.addClass("" + params.modifierClass + params.type + "-dynamic"), swiper.pagination.dynamicBulletIndex = 0, params.dynamicMainBullets < 1 && (params.dynamicMainBullets = 1)), "progressbar" === params.type && params.progressbarOpposite && $el.addClass(params.progressbarOppositeClass), params.clickable && $el.on("click", classesToSelector(params.bulletClass), (function onClick(e) {
                        e.preventDefault();
        ...
        Pagination$1, Scrollbar = {
            ...
            updateSize: function updateSize() {
                ...
                if (this.params.scrollbar.el && this.scrollbar.el) {
                    ...
                    var trackSize = this.isHorizontal() ? $el[0].offsetWidth : $el[0].offsetHeight,
                        ...
                        dragSize;
                    dragSize = "auto" === this.params.scrollbar.dragSize ? trackSize * divider : parseInt(this.params.scrollbar.dragSize, 10), this.isHorizontal() ? $dragEl[0].style.width = dragSize + "px" : $dragEl[0].style.height = dragSize + "px", $el[0].style.display = divider >= 1 ? "none" : "", this.params.scrollbar.hide && ($el[0].style.opacity = 0), extend(scrollbar, {
                        trackSize: trackSize,
                        divider: divider,
            ...
            init: function init() {
                ...
                if (this.params.scrollbar.el) {
                    ...
                    0 === $dragEl.length && ($dragEl = $('<div class="' + this.params.scrollbar.dragClass + '"></div>'), $el.append($dragEl)), extend(scrollbar, {
                        $el: $el,
                        el: $el[0],
                        $dragEl: $dragEl,
                        dragEl: $dragEl[0]
        ...
        Lazy$1, Controller = {
            ...
            setTranslate: function setTranslate(_setTranslate, byController) {
                var swiper = this,
                    ...
                    multiplier, controlledTranslate, Swiper = swiper.constructor;
        ...
        A11y$1, History = {
            ...
            getPathValues: function getPathValues(urlOverride) {
                var window = getWindow(),
                    location, pathArray = (location = urlOverride ? new URL(urlOverride) : window.location).pathname.slice(1).split("/").filter((function(part) {
                        return "" !== part
                    })),
        ...
        EffectFlip, Coverflow = {
            setTranslate: function setTranslate() {
                for (var swiper = this, swiperWidth = this.width, swiperHeight = this.height, slides = this.slides, slidesSizesGrid = this.slidesSizesGrid, params = this.params.coverflowEffect, isHorizontal = this.isHorizontal(), transform = this.translate, center = isHorizontal ? swiperWidth / 2 - transform : swiperHeight / 2 - transform, rotate = isHorizontal ? params.rotate : -params.rotate, translate = params.depth, i = 0, length = slides.length; i < length; i += 1) {
                    var $slideEl = slides.eq(i),
        ...
        EffectCoverflow, Thumbs = {
            init: function init() {
                ...
                return thumbsParams.swiper instanceof SwiperClass ? (this.thumbs.swiper = thumbsParams.swiper, extend(this.thumbs.swiper.originalParams, {
                    watchSlidesProgress: !0,
                    slideToClickedSlide: !1
                }), extend(this.thumbs.swiper.params, {
                    watchSlidesProgress: !0,
                    slideToClickedSlide: !1
                })) : isObject(thumbsParams.swiper) && (this.thumbs.swiper = new SwiperClass(extend({}, thumbsParams.swiper, {
        ...
        Thumbs$1, components = [Virtual$1, Keyboard$1, {
            name: "mousewheel",
            params: {
                mousewheel: {
                    enabled: !1,
            ...
            on: {
                ...
                activeIndexChange: function activeIndexChange(swiper) {
                    swiper.params.loop ? swiper.pagination.update() : void 0 === swiper.snapIndex && swiper.pagination.update()
                },
            ...
            on: {
                beforeInit: function beforeInit(swiper) {
                    if ("fade" === swiper.params.effect) {
                        ...
                        extend(swiper.params, overwriteParams), extend(swiper.originalParams, overwriteParams)
                    }
                },
                setTranslate: function setTranslate(swiper) {
